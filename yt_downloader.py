...
class YouTubeDownloaderApp:
    ...
    def start_dependency_check(self):
        ...
        try:
            ...
            self.logger.error(f"启动依赖检查失败: {str(e)}")
            self._show_fatal_error(f"启动依赖检查失败: {str(e)}")
    
    def setup_logging(self):
        """配置日志系统，将日志输出到GUI和文件"""
        try:
            self.logger = logging.getLogger(__name__)
            self.logger.setLevel(logging.DEBUG)  # 设置为DEBUG级别以获取更多信息
    ...
    def process_log_messages(self):
        ...
        try:
            ...
            # 如果日志处理失败，尝试记录到文件
            try:
                self.logger.error(f"日志处理失败: {str(e)}")
            except:
                pass  # 无法记录到日志，已经尽力了
